...
twitter_files_of_month = LOAD '/user/flume/keyword/tweets/2015/10/{01,02,03,04,05,06,07,08,09,10,11,23,24,25,26,27,28,29,30,31}' USING com.twitter.elephantbird.pig.load.JsonLoader('-nestedLoad') AS (json:map[]);
...
STORE initial_informationflow_filtered INTO '/project/201510/initial_informationflow' USING PigStorage('\t');
STORE clustering_features INTO '/project/201510/sequence_files' USING com.twitter.elephantbird.pig.store.SequenceFileStorage('-c com.twitter.elephantbird.pig.util.TextConverter', '-c com.twitter.elephantbird.pig.util.TextConverter');
STORE visualize_features2 INTO '/project/201510/tweet_detailed' USING PigStorage('\t');
STORE twitter_id_to_date INTO '/project/201510/twitter_id_date' USING com.twitter.elephantbird.pig.store.SequenceFileStorage('-c com.twitter.elephantbird.pig.util.TextConverter', '-c com.twitter.elephantbird.pig.util.TextConverter');
